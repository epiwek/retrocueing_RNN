...
This is the configuration file for Experiment 4, cue validity 1, AI vs learning speed analysis. It differs from the main
 Experiment 4, cue validity 1 configuration file in just one aspect - it is set to train more (N=100) models.

To run the experiment, pass the path to the folder which is to contain the data from all Experiments on lines 199-200.
...
PARAMS['expt_key'] = 'expt_4_cue_val_1'
...
PARAMS['cue_validity'] = 1  # proportion of trials on which the retro-cued and probed locations match
...
PARAMS['MSE_criterion'] = 0.0005  # loss threshold if the criterion is just some value of loss
