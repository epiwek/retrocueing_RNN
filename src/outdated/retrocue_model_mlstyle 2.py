...
def train_model(params,data,device):
    ...
    loss_valid = torch.zeros(n_valid_trials,params['n_epochs']).to(device)
    ...
    for i in range(params['n_epochs']):   
            ...
            for trial in range(params['batch_size']):
                ...
                if params['l2_activity']:
                    loss += torch.norm(torch.norm(o.squeeze(),dim=1))**2
            ...
            # if torch.sum((loss_all[valid_ix,i])) <= params['MSE_criterion']:
            ...
            if criterion_reached:
                ...
                net_outputs = net_outputs[:,:,:i]
                loss_all = loss_all[:,:i]
                shuffling_order = shuffling_order[:,:i]
                break
...
def get_dLoss_dt(loss_vals,opt = 'non-smoothed',**kwargs):
    '''
    Calculate the derivative of the loss function wrt time. Used for finding 
    learning plateaus. Note - this is a low-level implementation, so will be 
    slow. There is probably a pytorch way of doing it.
    ...
    opt : str, optional
        'non-smoothed' or 'smoothed'
        whether or not to apply a Gaussian filter to smooth the loss curve. 
        The default is 'non-smoothed'.
    **kwargs : TYPE
        smooth_window: int
            number of timepoints to be used by the Gaussian filter
    ...
    loss_clean : np.array
        loss values summed over a batch and after optional smoothing

    '''
    
    loss_summed = torch.sum(loss_vals,0) # sum the loss across all trials within an epoch
    if opt == 'smoothed':
        # convolve with a Gaussian filter to smooth the loss curve
        loss_clean = gaussian_filter1d(loss_summed,smooth_window)
    else:
        loss_clean = loss_summed
    
    # calculate the derivative
    dLoss = np.zeros(loss_summed.shape[0]-1)
    for i in range(loss_summed.shape[0]-1):
            dLoss[i] = loss_clean[i+1] - loss_clean[i]
    return dLoss, loss_clean

def apply_conv_criterion(loss_vals,params):
    '''
    ...
    dLoss,loss_clean = get_dLoss_dt(loss_vals,params)
    ...
    ix2 = np.where(loss_clean<params['conv_criterion']['cond2'])[0]
