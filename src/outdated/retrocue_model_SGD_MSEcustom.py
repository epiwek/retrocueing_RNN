...
def train_model(params,data,device):
    ...
    loss_all = torch.empty(params['stim_set_size'],params['n_epochs']).to(device)
    
    n_valid_trials = int(params['cue_validity']*params['stim_set_size'])
    loss_valid = torch.empty(n_valid_trials,params['n_epochs']).to(device)
    ...
    for i in range(params['n_epochs']):   
            ...
            # else:
            #     criterion_reached = apply_conv_criterion(params,loss_valid[:,i-window+1:i+1])
            #
...
def custom_MSE_loss(params,output,target):
    ...
    loss *= 1000 # rescale the loss so it's easier to interpret values
    return loss
