...
n_bins = 4
n_conditions = n_bins*n_bins*2
M = n_bins*2


rdm_precue = np.zeros((n_conditions,n_conditions,len(converged)))
rdm_postcue = np.zeros((n_conditions,n_conditions,len(converged)))

rdm_precue_binned = np.zeros((M,M,len(converged)))
rdm_postcue_binned = np.zeros((M,M,len(converged)))
rdm_stimuli_binned = np.zeros((M,M,len(converged)))
...
for i,model_number in enumerate(converged):
    ...
    rdm_precue[:,:,i] = squareform(pdist(rdm_data[:,5,:],'correlation'))
    rdm_postcue[:,:,i] = squareform(pdist(rdm_data[:,11,:],'correlation'))
    
    f = open(load_path+'/pca_data_model' + model_number + '.pckl', 'rb')
    [trial_data,delay1,delay2] = pickle.load(f)
    rdm_data = obj
    f.close()
    rdm_stimuli_binned[:,:,i] = squareform(pdist(trial_data[:,:,:,0].squeeze().T,'correlation'))
    rdm_precue_binned[:,:,i] = squareform(pdist(delay1,'correlation'))
    rdm_postcue_binned[:,:,i] = squareform(pdist(delay2,'correlation'))
...
pickle.dump(indiv_model_rdms,open(base_path+'RSA/indiv_model_rdms.pckl','wb'))
...
pickle.dump(pre_data_RDM_averaged,open(base_path+'RSA/pre_data_RDM_averaged.pckl','wb'))
pickle.dump(post_data_RDM_averaged,open(base_path+'RSA/post_data_RDM_averaged.pckl','wb'))
...
plt.subplot(121)
plt.imshow(np.mean(rdm_precue,2))
plt.xticks([])
plt.yticks([])
...
plt.subplot(122)
plt.imshow(np.mean(rdm_postcue,2))
plt.colorbar()
plt.title('post-cue')
