...
def get_plane_angle_cued_across_delays(constants,cv=2):
    '''
    For each location, get the plane ange between its subspaces from the 
    ...
    theta_ad_train = np.zeros((2,constants.PARAMS['n_models'],cv))
    theta_ad_test = np.zeros((2,constants.PARAMS['n_models'],cv))
    
    psi_ad_train = np.zeros((2,constants.PARAMS['n_models'],cv))
    ...
    PVE_3D = np.zeros((2,2,constants.PARAMS['n_models'])) # cv folds, unrotated/rotated, model
    ...
    for model in range(constants.PARAMS['n_models']):
        ...
        for i in range(2):
            ...
            same_ixs[model,i] = np.argmax([np.abs(np.cos(np.radians(loc1_subspace['theta']))),
    ...
    theta_unrotated_mean = np.abs(np.array([theta_ad_test[0,m,:].mean(-1) if np.sign(theta_ad_test[0,m,:]).sum()!=0 else np.abs(theta_ad_test[0,m,:]).mean(-1) for m in range(constants.PARAMS['n_models'])]))
    theta_rotated_mean = np.abs(np.array([theta_ad_test[1,m,:].mean(-1) if np.sign(theta_ad_test[1,m,:]).sum()!=0 else np.abs(theta_ad_test[1,m,:]).mean(-1) for m in range(constants.PARAMS['n_models'])]))
    psi_unrotated_mean = np.array([psi_ad_test[0,m,:].mean(-1) if np.sign(psi_ad_test[0,m,:]).sum()!=0 else np.abs(psi_ad_test[0,m,:]).mean(-1) for m in range(constants.PARAMS['n_models'])])
    psi_rotated_mean = np.array([psi_ad_test[1,m,:].mean(-1) if np.sign(psi_ad_test[1,m,:]).sum()!=0 else np.abs(psi_ad_test[1,m,:]).mean(-1) for m in range(constants.PARAMS['n_models'])])
...
def get_CDI(constants):
    ...
    for model in range(constants.PARAMS['n_models']):
        ...
        for cued_loc_ix,cued_loc_3D_coords in enumerate([cued_up_3Dcoords,cued_down_3Dcoords]):
            ...
            CDI[model,cued_loc_ix,0,0,0] = quadrilatArea(cued_loc_3D_coords[:n_colours,:])
            CDI[model,cued_loc_ix,0,1,0] = quadrilatArea(cued_loc_3D_coords[n_colours:n_colours*2,:])
            #post-cue, valid trials
            CDI[model,cued_loc_ix,1,0,0] = quadrilatArea(cued_loc_3D_coords[n_colours*2:n_colours*3,:])
            CDI[model,cued_loc_ix,1,1,0] = quadrilatArea(cued_loc_3D_coords[n_colours*3:n_colours*4,:])
            
            if constants.PARAMS['experiment_number'] == 3:
                # add the probe timepoints and invalid trials 
                
                #post-probe, valid trials
                CDI[model,cued_loc_ix,2,0,0] = quadrilatArea(cued_loc_3D_coords[n_colours*4:n_colours*5,:])
                CDI[model,cued_loc_ix,2,1,0] = quadrilatArea(cued_loc_3D_coords[n_colours*5:n_colours*6,:])
                
                if constants.PARAMS['cue_validity'] < 1:
                    # pre-cue, invalid trials
                    CDI[model,cued_loc_ix,0,0,1] = quadrilatArea(cued_loc_3D_coords[n_colours*6:n_colours*7,:])
                    CDI[model,cued_loc_ix,0,1,1] = quadrilatArea(cued_loc_3D_coords[n_colours*7:n_colours*8,:])
                    #post-cue, invalid trials
                    CDI[model,cued_loc_ix,1,0,1] = quadrilatArea(cued_loc_3D_coords[n_colours*8:n_colours*9,:])
                    CDI[model,cued_loc_ix,1,1,1] = quadrilatArea(cued_loc_3D_coords[n_colours*9:n_colours*10,:])
                    #post-probe, invalid trials
                    CDI[model,cued_loc_ix,2,0,1] = quadrilatArea(cued_loc_3D_coords[n_colours*10:n_colours*11,:])
                    CDI[model,cued_loc_ix,2,1,1] = quadrilatArea(cued_loc_3D_coords[n_colours*11:n_colours*12,:])
...
def plot_CDI(constants,CDI,logTransform=True,save_fig=True):
    ...
    if constants.PARAMS['experiment_number']!=3:
        # plot individua model datapoints
        for model in range(0,constants.PARAMS['n_models']):
            ...
            # want to add datapoints for expt3, but this makes the pot look 
            # really messy
            if constants.PARAMS['experiment_number']==3:
                # lines
...
def get_cued_subspaces_indivModels(constants,trial_type='valid'):
    '''
