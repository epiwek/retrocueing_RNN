...
def plot_cued_subspaces(constants):
    # ths needs to be updated
    load_path = constants.PARAMS['FULL_PATH'] + 'pca_data/valid_trials/'
    n_colours = constants.PARAMS['B']
...
def get_all_binned_data(constants, trial_type='valid'):
    ...
    for model in range(constants.PARAMS['n_models']):
        ...
        for geometry, f_name in zip(geometry_names,geometry_f_names):
            with open(load_path + f_name + 'model' + str(model) + '.pckl', 'rb') as f:
                all_data[str(model)][geometry] = pickle.load(f)
    return geometry_names, all_data


def model_geometry_looper(constants, all_data, geometry_name, delay_name):
    all_subspaces = {}
    all_psi, all_theta, all_PVEs = [], [], []
    for model in range(constants.PARAMS['n_models']):
        model_data = all_data[str(model)][geometry_name][delay_name]
        # fit the geometry to data from model
        all_subspaces[str(model)] = Geometry(model_data, constants)
        all_subspaces[str(model)].get_geometry()

        # extract the angle measures
        all_psi.append(all_subspaces[str(model)].psi_degrees)
        all_theta.append(all_subspaces[str(model)].theta_degrees)
        all_PVEs.append(all_subspaces[str(model)].PVEs)
    return all_subspaces, all_psi, all_theta, all_PVEs

def run_cued_geom_analysis(constants, all_data):
    # get the Cued item geometry for all trained networks


    cued_subspaces, psi, theta, PVEs = [], [], [], []

    for delay in range(constants.PARAMS['n_delays']):
        cued_subspaces.append([])
        psi.append([])
        theta.append([])
        PVEs.append([])
        cued_subspaces[delay], psi[delay], theta[delay], PVEs[delay] = model_geometry_looper(constants, all_data, 'cued', 'delay'+str(delay+1))
...
def run_full_rep_geom_analysis(constants):
    ...
    # get the uncued geometry
    run_uncued_geom_analysis(constants, all_data)

    # get the cued/uncued geometry
