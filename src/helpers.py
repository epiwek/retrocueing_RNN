...
def bin_data(params, data, custom_B=None):
    """
    ...
    if len(data.shape) == 2:
        # matrix with a single timepoint
        data_binned = data.unsqueeze(0).reshape((M, n_samples, params['n_rec']))
    elif len(data.shape) == 3:
        # matrix with multiple timepoints
        n_timepoints = data.shape[1]
        data_binned = data.unsqueeze(0).reshape((M, n_samples, n_timepoints, params['n_rec']))
    else:
        raise ValueError('Data array should be at least 2-dimensional.')
...
def bin_labels(labels_list, n_bins):
    ...
    # new label assignments for each old label
    ...
    labels_list_integer = np.array([np.where(labels == val)[0] for ix, val in enumerate(np.array(labels_list))])
...
def transfer_to_cpu(constants):
    ...
    """
    # imports allow the function to be easily used outside the main.py script
...
def circ_mean(angles):
    ...
    ang_mean = vec_to_angle(cart_mean)
    return ang_mean
