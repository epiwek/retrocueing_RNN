...
def plot_plane_angles_multiple(constants,angles_radians,r=None,paired=True,cu=False,expt3=False,custom_labels=[]):
    if np.all(r == None):
        # if r == None
...
def get_inf_stats_angles_cued_vs_uncued(angles_radians):
    # need to stretch the data to the [0,360] range by 
    # multiplying by 2, then test for significant clustering at 90 degrees
...
def run_plane_angles_analysis_rotated_unrotated(constants):
    ...
    plot_plane_angles_multiple(constants,theta_angles_radians,r=None,paired=False,cu=True,expt3=False,custom_labels=['unrotated','rotated'])
    ...
    plot_plane_angles_multiple(constants,psi_angles_radians,r=None,paired=False,cu=True,expt3=False,custom_labels=['unrotated','rotated'])
    ...
    pycircstat.rayleigh(np.radians(psi_angles_radians[nonnan_ix,0]))
    np.degrees(pycircstat.descriptive.mean(np.radians(psi_angles_radians[nonnan_ix,0])))
    
    nonnan_ix = np.where(np.invert(np.isnan(psi_angles_radians[:,1])))[0]
    pycircstat.rayleigh(np.radians(psi_angles_radians[nonnan_ix,1]))
    np.degrees(pycircstat.descriptive.mean(np.radians(psi_angles_radians[nonnan_ix,1])))
