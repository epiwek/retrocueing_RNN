...
if (model_type == 'RNN'):
    ...
    load_path = '/Users/emilia/OneDrive - Nexus365/DPHIL/PROJECTS/retrocue_RNN_project/shared_repo/retrocue_rnn/emilia/'+\
        'data_vonMises/MSELoss/with_fixation_12cycles_noise/epsilon0.2/kappa1.0/nrec200/lr0.005/pca_data'
...
converged = np.arange(1)


for i,model_number in enumerate(converged):
# for model_number in np.arange(8,9):
    ...
    if (model_type == 'RNN'): 
        ...
        plot_subspace(ax,np.random.permutation(delay1_3dcoords[:n_colours,:]),delay1_planeUp.components_,fc='k',a=0.2)
        plot_subspace(ax,np.random.permutation(delay1_3dcoords[n_colours:,:]),delay1_planeDown.components_,fc='k',a=0.2)
    ...
    if (model_type == 'RNN'): 
        ...
        plot_subspace(ax2,np.random.permutation(delay2_3dcoords[:n_colours,:]),delay2_planeUp.components_,fc='k',a=0.2)
        plot_subspace(ax2,np.random.permutation(delay2_3dcoords[n_colours:,:]),delay2_planeDown.components_,fc='k',a=0.2)
    ...
    plt.savefig(load_path+'/SI_vs_delay1_fig_'+model_type+model_number)
    ...
    path = load_path + '/angles/angles_' + model_type + model_number + '.pckl'
    pickle.dump(angles,open(path,'wb'))
    
    pca3 = [delay1_pca, delay2_pca]
    path = load_path + '/pca3/pca3_' + model_type + model_number + '.pckl'
    pickle.dump(pca3,open(path,'wb'))
    
    pca2 = [delay1_planeUp,delay1_planeDown,delay2_planeUp,delay2_planeDown]
    path = load_path + '/pca2/pca2_' + model_type + model_number + '.pckl'
    ...
    path = load_path + '/planes/planes_' + model_type + model_number + '.pckl'
    pickle.dump(planes,open(path,'wb'))
    
    coords3D = [delay1_3dcoords , delay2_3dcoords]
    path = load_path + '/pca3/coords3D_' + model_type + model_number + '.pckl'
