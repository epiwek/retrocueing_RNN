...
def sort_by_uncued(data_struct,params,key='data'):
    ...
    key : str, optional
         Keyword for the data_struct udictionary, nder which the sorted data 
    ...
    n_trials = data_struct[key].shape[0]
...
def bin_data(data,params):
    '''
    Bin the trials into M colour bins. M is extracted from the experiment 
    parameters.
    ...
    n_samples = data.shape[0]//params['M']
    if len(data.shape) == 2:
        # matrix with single timepoint
        data_binned = data.unsqueeze(0).reshape((params['M'],n_samples,
                                params['n_rec']))
