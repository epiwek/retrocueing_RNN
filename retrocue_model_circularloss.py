...
def train_model(params,data,device):
    ...
    for ix,i in enumerate(epochs):
            ...
            # circular mean over epoch - only valid trials
            loss_epoch[i] = helpers.circ_mean(loss_all[valid_ix,i]).abs()
            ...
            if i<window:
                ...
                # criterion_reached = apply_conv_criterion(params,loss_epoch[i-window+1:i+1])
                criterion_reached = False
...
def save_model(path,params,model,track_training):
    ...
    # main_path = path +'sigma' + str(params['sigma'])+\
...
def eval_model(model,test_data,params,save_path):
    ...
    # pca uncued - like above, binned and averaged across cued
    ...
    # labels_uncued_binned = helpers.bin_labels(labels_uncued,params['B'])
    
    # delay1_uncued = eval_data['data'][:,d1_ix,:]
    # delay2_uncued = eval_data['data'][:,d2_ix,:]

    # delay1_uncued_down = torch.zeros((params['B'],params['n_rec'])) # c2 uncued
    # delay1_uncued_up = torch.zeros((params['B'],params['n_rec'])) # c1 uncued
    
    # delay2_uncued_down = torch.zeros((params['B'],params['n_rec'])) # c2 uncued
    # delay2_uncued_up = torch.zeros((params['B'],params['n_rec'])) # c1 uncued
    
    
    # for colour in range(params['B']):
    #     ix = np.where(labels_uncued_binned==colour)[0]
    #     ix_down = ix[np.where(ix<n_trials//2)[0]]
    #     ix_up = ix[np.where(ix>=n_trials//2)[0]]
    
    #     delay2_uncued_down[colour,:] = delay2_uncued[ix_down,:].mean(0)
    #     delay2_uncued_up[colour,:] = delay2_uncued[ix_up,:].mean(0)
        
    #     delay1_uncued_down[colour,:] = delay1_uncued[ix_down,:].mean(0)
    #     delay1_uncued_up[colour,:] = delay1_uncued[ix_up,:].mean(0)
    ...
    #         'delay1':torch.cat((delay1_uncued_down,delay1_uncued_up)),
    #         'delay2':torch.cat((delay2_uncued_down,delay2_uncued_up)),
